...
static int len = std::snprintf(USAGE, sizeof(USAGE),
                               ""
                               "Minesweeper (%s)\n\
\n\
Usage:\n\
...
public:
  Board(int difficulty) {
    ...
    // Construct boards
    _mine_board =
        std::vector<std::vector<short>>(_side, std::vector<short>(_side, 0));
    _board =
        std::vector<std::vector<char>>(_side, std::vector<char>(_side, '-'));
  ...
  void printMineBoard() { _printBoard(_mine_board, 'i'); }

  void printPlayerBoard() { _printBoard(_board, 'c'); }

  bool isValid(int row, int col) {
    return row >= 0 && col >= 0 && row < _side && col < _side;
  }

  int getMines() { return _mines; }

private:
  int _side;
  int _mines;
  std::vector<std::vector<short>> _mine_board;
   ...
   */
  template <class T>
  void _printBoard(std::vector<std::vector<T>> board, char type) {
    // Print col numbers
    printf("    ");
